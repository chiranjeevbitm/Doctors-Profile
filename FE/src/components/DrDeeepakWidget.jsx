...
// ─── HELPERS ──────────────────────────────────────────────────────────────
function buildWhatsAppMessage(answers, t) {
  return encodeURIComponent(
    t('whatsappMsg') + `\n\n` +
    `🩺 *${t('concern')}:* ${answers.concern}\n\n` +
    t('instantResponse')
  );
}
...
export default function DrDeeepakWidget() {
  ...
  const STEPS = [
    {
      id: 'concern',
      bot: t('helloAssistant'),
      chips: [t('fever'), t('coldCough'), t('thyroid'), t('others')],
      type: 'chips',
    },
    {
      id: 'other_concern',
  ...
  useEffect(() => {
    if (!open) {
      const timer = setTimeout(() => setUnread(true), 4000);
  ...
  const finishChat = (finalAnswers) => {
    ...
    pushBot(
      `✅ ${t('thanksSummary')}\n\n` +
      `🩺 ${t('concern')}: ${finalAnswers.concern}\n\n` +
      `${t('sendSummary')}`
    );
  };
  ...
  const preConsultLabel = `${t('preConsultForm')} · ${STEPS.length} ${t('quickQuestions')}`;
            ...
            <div style={{
              ...
              {CONTACTS.map((contact) => (
                <button
                  key={contact.phone}
                  onClick={() => {
                    const msg = buildWhatsAppMessage(answers, t);
